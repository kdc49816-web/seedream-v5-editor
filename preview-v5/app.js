(() => {
  ...
  const state = {
    ...
    albumView: 'grid', albumStackIndex: 0, dragId: null, toastTimer: null
  };
  ...
  // ---- Smooth album stack engine: fixed DOM pool, no per-swipe re-render ----
  const stackRuntime = {
    els:null, animating:false, active:false, sx:0, sy:0, dx:0, dy:0,
    moved:false, started:0, raf:0, pending:null, lastTap:0, urls:new Map()
  };
  const stackRoles=['role-current','role-next1','role-next2','role-next3','role-prev','current'];
  const stackMod=(n,m)=>((n%m)+m)%m;
  const stackIdx=(off)=>stackMod(state.albumStackIndex+off,state.album.length);

  function stackSrc(item){
    if(!item)return '';
    if(item.blob){
      if(stackRuntime.urls.has(item.id)) return stackRuntime.urls.get(item.id);
      const u=URL.createObjectURL(item.blob); stackRuntime.urls.set(item.id,u); return u;
    }
    return item.url||item.dataUrl||'';
  }
  function stackPruneUrls(){
    const ids=new Set(state.album.map(x=>x.id));
    for(const [id,u] of stackRuntime.urls){if(!ids.has(id)){URL.revokeObjectURL(u);stackRuntime.urls.delete(id)}}
  }
  function stackCreateCard(){
  ...
  function stackEnsurePool(){
    const deck=$('#albumStackDeck'); if(!deck)return false;
    if(stackRuntime.els?.current?.isConnected)return true;
    deck.replaceChildren();
    const next3=stackCreateCard(), next2=stackCreateCard(), next1=stackCreateCard(), prev=stackCreateCard(), current=stackCreateCard();
    ...
    stackRuntime.els={current,next1,next2,next3,prev,deck};
    deck.addEventListener('pointerdown',stackPointerDown,{passive:true});
    deck.addEventListener('pointermove',stackPointerMove,{passive:false});
    deck.addEventListener('pointerup',stackPointerUp,{passive:false});
    deck.addEventListener('pointercancel',stackPointerCancel,{passive:true});
    deck.addEventListener('dblclick',e=>{if(e.target.closest('.role-current .stack-card-shell')){e.preventDefault();openLightbox(stackSrc(state.album[state.albumStackIndex]));}});
    return true;
  }
  function stackRole(el,name){
    stackRoles.forEach(c=>el.classList.remove(c)); el.classList.add('stack-card',name);
    if(name==='role-current')el.classList.add('current');
    el.style.transform='';el.style.opacity='';el.style.pointerEvents='';
  }
  function stackHydrate(el,item,absoluteIndex){
    if(!el||!item)return;
    const img=$('.stack-image',el), strong=$('.stack-meta strong',el), span=$('.stack-meta span',el);
    const src=stackSrc(item);
    if(img.dataset.itemId!==String(item.id)){
      img.dataset.itemId=String(item.id); img.src=src; img.decoding='async';
      img.decode?.().catch(()=>{});
    }
    strong.textContent=item.name||'相册图片'; span.textContent='第 '+(absoluteIndex+1)+' 张';
  }
  function stackResetRoles(){
    const e=stackRuntime.els;if(!e)return;
    stackRole(e.current,'role-current');stackRole(e.next1,'role-next1');stackRole(e.next2,'role-next2');stackRole(e.next3,'role-next3');stackRole(e.prev,'role-prev');
  }
  function stackHydrateAll(){
    if(!state.album.length||!stackRuntime.els)return;
    const e=stackRuntime.els;
    stackHydrate(e.current,state.album[stackIdx(0)],stackIdx(0));
    stackHydrate(e.next1,state.album[stackIdx(1)],stackIdx(1));
    stackHydrate(e.next2,state.album[stackIdx(2)],stackIdx(2));
    stackHydrate(e.next3,state.album[stackIdx(3)],stackIdx(3));
    stackHydrate(e.prev,state.album[stackIdx(-1)],stackIdx(-1));
    $('#albumStackCounter').textContent=(state.albumStackIndex+1)+' / '+state.album.length;
    $('#stackPrevBtn').disabled=state.album.length<=1;$('#stackNextBtn').disabled=state.album.length<=1;
  }
  function stackPredecode(){
    if(!state.album.length)return;
    for(let o=-2;o<=5;o++){
      const item=state.album[stackIdx(o)], src=stackSrc(item); if(!src)continue;
      const im=new Image();im.decoding='async';im.src=src;im.decode?.().catch(()=>{});
    }
  }
  function renderAlbumStack(){
    stackPruneUrls();
    if(!state.album.length){
      const deck=$('#albumStackDeck');deck.innerHTML='<div class="empty-state"><strong>相册还是空的</strong><span>先上传几张图片再切到卡片模式。</span></div>';
      stackRuntime.els=null;$('#albumStackCounter').textContent='0 / 0';$('#stackPrevBtn').disabled=true;$('#stackNextBtn').disabled=true;return;
    }
    state.albumStackIndex=stackMod(state.albumStackIndex,state.album.length);
    stackEnsurePool();stackResetRoles();stackHydrateAll();stackPredecode();
  }
  function stackTf(x,y,s=1,r=0){return `translate3d(calc(-50% + ${x}px),calc(-50% + ${y}px),0) scale(${s}) rotate(${r}deg)`}
  function stackApplyFrame(){
    stackRuntime.raf=0;
    const rt=stackRuntime,e=rt.els;if(!rt.pending||!rt.active||rt.animating||!e)return;
    const x=rt.pending.x,y=rt.pending.y,ax=Math.abs(x),p=Math.min(1,ax/130);
    e.current.style.transform=stackTf(x,Math.max(-14,Math.min(14,y*.07)),1,x/31);
    e.current.style.opacity=String(Math.max(.64,1-ax/580));
    if(x>0){
      e.next1.style.transform=stackTf(14*(1-p),18*(1-p),.965+.035*p);e.next1.style.opacity=String(.96+.04*p);
      e.next2.style.transform=stackTf(27-13*p,34-16*p,.93+.035*p);e.next2.style.opacity=String(.89+.07*p);
      e.next3.style.transform=stackTf(39-12*p,49-15*p,.90+.03*p);e.next3.style.opacity=String(.72+.17*p);
    }else if(x<0){
      e.prev.style.transform=stackTf(-18*(1-p),18*(1-p),.965+.035*p);e.prev.style.opacity=String(p);
    }
  }
  function stackPointerDown(e){
    const rt=stackRuntime;
    if(state.albumView!=='stack'||rt.animating||state.album.length<=1||!e.target.closest('.role-current .stack-card-shell'))return;
    rt.active=true;rt.moved=false;rt.dx=rt.dy=0;rt.sx=e.clientX;rt.sy=e.clientY;rt.started=performance.now();
  }
  function stackPointerMove(e){
    const rt=stackRuntime;if(!rt.active||rt.animating)return;
    rt.dx=e.clientX-rt.sx;rt.dy=e.clientY-rt.sy;if(Math.abs(rt.dx)>5)rt.moved=true;
    if(Math.abs(rt.dx)>Math.abs(rt.dy)){
      e.preventDefault();rt.pending={x:rt.dx,y:rt.dy};
      if(!rt.raf)rt.raf=requestAnimationFrame(stackApplyFrame);
    }
  }
  function stackClearInline(){
    const e=stackRuntime.els;if(!e)return;
    [e.current,e.next1,e.next2,e.next3,e.prev].forEach(el=>{el.style.transform='';el.style.opacity=''});
  }
  async function stackSpring(){
    const rt=stackRuntime,e=rt.els;if(!e)return;
    const x=rt.dx,y=rt.dy,ax=Math.abs(x),p=Math.min(1,ax/130);
    const opts={duration:300,easing:'cubic-bezier(.22,1,.36,1)',fill:'forwards'};
    const jobs=[e.current.animate([{transform:stackTf(x,Math.max(-14,Math.min(14,y*.07)),1,x/31),opacity:Math.max(.64,1-ax/580)},{transform:stackTf(0,0),opacity:1}],opts)];
    if(x>0){
      jobs.push(e.next1.animate([{transform:stackTf(14*(1-p),18*(1-p),.965+.035*p)},{transform:stackTf(14,18,.965)}],opts));
      jobs.push(e.next2.animate([{transform:stackTf(27-13*p,34-16*p,.93+.035*p)},{transform:stackTf(27,34,.93)}],opts));
      jobs.push(e.next3.animate([{transform:stackTf(39-12*p,49-15*p,.90+.03*p)},{transform:stackTf(39,49,.90)}],opts));
    }else if(x<0){
      jobs.push(e.prev.animate([{transform:stackTf(-18*(1-p),18*(1-p),.965+.035*p),opacity:p},{transform:stackTf(-18,18,.965),opacity:0}],opts));
    }
    await Promise.all(jobs.map(a=>a.finished.catch(()=>{})));jobs.forEach(a=>a.cancel());stackClearInline();
  }
  async function stackNext(){
    const rt=stackRuntime,e=rt.els;if(rt.animating||state.album.length<=1||!e)return;
    rt.animating=true;const w=$('.stack-viewport')?.clientWidth||window.innerWidth;
    const x=rt.dx>0?rt.dx:0,y=rt.dy||0,p=Math.min(1,Math.abs(x)/130);
    const opts={duration:285,easing:'cubic-bezier(.18,.82,.23,1)',fill:'forwards'};
    const jobs=[
      e.current.animate([{transform:stackTf(x,Math.max(-14,Math.min(14,y*.07)),1,x/31),opacity:Math.max(.64,1-Math.abs(x)/580)},{transform:stackTf(w+170,0,1,12),opacity:0}],opts),
      e.next1.animate([{transform:stackTf(14*(1-p),18*(1-p),.965+.035*p),opacity:.96+.04*p},{transform:stackTf(0,0,1),opacity:1}],opts),
      e.next2.animate([{transform:stackTf(27-13*p,34-16*p,.93+.035*p),opacity:.89+.07*p},{transform:stackTf(14,18,.965),opacity:.96}],opts),
      e.next3.animate([{transform:stackTf(39-12*p,49-15*p,.90+.03*p),opacity:.72+.17*p},{transform:stackTf(27,34,.93),opacity:.89}],opts)
    ];
    await Promise.all(jobs.map(a=>a.finished.catch(()=>{})));jobs.forEach(a=>a.cancel());
    state.albumStackIndex=stackIdx(1);
    const oldCurrent=e.current,oldPrev=e.prev;
    e.current=e.next1;e.next1=e.next2;e.next2=e.next3;e.next3=oldPrev;e.prev=oldCurrent;
    stackResetRoles();stackHydrate(e.next3,state.album[stackIdx(3)],stackIdx(3));stackHydrate(e.prev,state.album[stackIdx(-1)],stackIdx(-1));
    stackClearInline();stackHydrateAll();stackPredecode();rt.animating=false;
  }
  async function stackPrev(){
    const rt=stackRuntime,e=rt.els;if(rt.animating||state.album.length<=1||!e)return;
    rt.animating=true;const w=$('.stack-viewport')?.clientWidth||window.innerWidth;
    const x=rt.dx<0?rt.dx:0,y=rt.dy||0,p=Math.min(1,Math.abs(x)/130);
    const opts={duration:285,easing:'cubic-bezier(.18,.82,.23,1)',fill:'forwards'};
    const jobs=[
      e.current.animate([{transform:stackTf(x,Math.max(-14,Math.min(14,y*.07)),1,x/31),opacity:Math.max(.64,1-Math.abs(x)/580)},{transform:stackTf(-(w+170),0,1,-12),opacity:0}],opts),
      e.prev.animate([{transform:stackTf(-18*(1-p),18*(1-p),.965+.035*p),opacity:p},{transform:stackTf(0,0,1),opacity:1}],opts)
    ];
    await Promise.all(jobs.map(a=>a.finished.catch(()=>{})));jobs.forEach(a=>a.cancel());
    state.albumStackIndex=stackIdx(-1);
    const oldCurrent=e.current,oldNext3=e.next3,oldPrev=e.prev;
    e.current=oldPrev;e.next3=e.next2;e.next2=e.next1;e.next1=oldCurrent;e.prev=oldNext3;
    // hide old current before moving it behind, then fade it in behind the new current
    oldCurrent.style.opacity='0';oldCurrent.style.transform=stackTf(14,18,.965);
    stackResetRoles();stackHydrate(e.prev,state.album[stackIdx(-1)],stackIdx(-1));stackHydrateAll();
    oldCurrent.style.opacity='0';
    const fade=oldCurrent.animate([{opacity:0},{opacity:.96}],{duration:120,easing:'ease-out',fill:'forwards'});
    await fade.finished.catch(()=>{});fade.cancel();stackClearInline();stackPredecode();rt.animating=false;
  }
  async function stackPointerUp(e){
    const rt=stackRuntime;if(!rt.active||rt.animating)return;
    rt.active=false;if(rt.raf){cancelAnimationFrame(rt.raf);rt.raf=0}rt.pending=null;
    const elapsed=Math.max(1,performance.now()-rt.started),velocity=Math.abs(rt.dx)/elapsed;
    const swipe=(Math.abs(rt.dx)>68||(Math.abs(rt.dx)>34&&velocity>.38))&&Math.abs(rt.dx)>Math.abs(rt.dy);
    if(swipe){if(rt.dx>0)await stackNext();else await stackPrev();return}
    await stackSpring();
    if(!rt.moved){
      const now=Date.now();if(e.pointerType!=='mouse'&&now-rt.lastTap<330){openLightbox(stackSrc(state.album[state.albumStackIndex]));rt.lastTap=0}else rt.lastTap=now;
    }
  }
  function stackPointerCancel(){const rt=stackRuntime;if(!rt.active)return;rt.active=false;if(rt.raf){cancelAnimationFrame(rt.raf);rt.raf=0}rt.pending=null;stackSpring()}
  function stackProgrammatic(delta){const rt=stackRuntime;if(rt.animating||state.album.length<=1)return;rt.dx=delta>0?1:-1;rt.dy=0;delta>0?stackNext():stackPrev();}
  ...
  function renderAlbum(){
    ...
    $('#albumEmpty').classList.toggle('hidden',state.album.length>0);
    const isStack=state.albumView==='stack';
    $('#albumGrid').classList.toggle('hidden',isStack);
    $('#albumStackView').classList.toggle('hidden',!isStack);
    $('#toggleArrangeBtn').disabled=isStack;
    if(isStack){
      $('#arrangeTip').classList.add('hidden');$('#albumPagination').classList.add('hidden');
      renderAlbumStack();return;
    }
    revokeTempUrls();
    const totalPages=Math.max(1,Math.ceil(state.album.length/ALBUM_PAGE_SIZE)); state.albumPage=Math.min(Math.max(1,state.albumPage),totalPages);
    const start=(state.albumPage-1)*ALBUM_PAGE_SIZE, visible=state.album.slice(start,start+ALBUM_PAGE_SIZE), grid=$('#albumGrid');
    grid.classList.toggle('arranging',state.arranging); grid.innerHTML=visible.map(albumCard).join('');
    $('#albumPagination').classList.toggle('hidden',state.album.length<=ALBUM_PAGE_SIZE); $('#pageInfo').textContent=`${state.albumPage} / ${totalPages}`;
    $('#prevPageBtn').disabled=state.albumPage<=1; $('#nextPageBtn').disabled=state.albumPage>=totalPages;
    $('#arrangeTip').classList.toggle('hidden',!state.arranging); $('#toggleArrangeBtn').textContent=state.arranging?'完成整理':'整理顺序';
    bindZoomables(grid); bindAlbumDrag();
  }
  async function normalizeAlbumOrder(){ state.album.forEach((x,i)=>x.order=i); await Promise.all(state.album.map(x=>dbPut('album',x))); state.albumStackIndex=Math.min(state.albumStackIndex,Math.max(0,state.album.length-1)); }
  ...
  function bindEvents(){
    ...
    $('#albumUpload').addEventListener('change',e=>{uploadAlbumFiles(e.target.files);e.target.value='';}); $('#toggleArrangeBtn').addEventListener('click',()=>{if(state.albumView==='stack')return;state.arranging=!state.arranging;renderAlbum();}); $('#albumViewSwitch').addEventListener('click',e=>{const b=e.target.closest('[data-album-view]');if(b)setAlbumView(b.dataset.albumView);}); $('#stackPrevBtn').addEventListener('click',()=>stackProgrammatic(-1)); $('#stackNextBtn').addEventListener('click',()=>stackProgrammatic(1));
    $('#albumGrid').addEventListener('click',async e=>{if(e.target.closest('.drag-handle'))return;const btn=e.target.closest('[data-action]');if(!btn)return;const card=e.target.closest('.media-card'),id=card?.dataset.id,item=state.album.find(x=>x.id===id);if(!item)return;const a=btn.dataset.action;if(a==='open')openLightbox(trackedSrc(item));if(a==='delete'&&confirm('从相册删除这张图片？')){await dbDelete('album',id);state.album=state.album.filter(x=>x.id!==id);state.albumStackIndex=Math.min(state.albumStackIndex,Math.max(0,state.album.length-1));await normalizeAlbumOrder();renderCounts();renderAlbum();}if(a==='left')await moveAlbum(id,-1);if(a==='right')await moveAlbum(id,1);});
  ...
  window.addEventListener('beforeunload',()=>{revokeTempUrls();for(const u of stackRuntime.urls.values())URL.revokeObjectURL(u);stackRuntime.urls.clear();}); init().catch(e=>{console.error(e);toast('页面初始化失败，请刷新重试');});
})();
